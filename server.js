...
const Action = mongoose.model("Action", new mongoose.Schema({
  ...
  type: {
    type: String,
    enum: ["create_tontine", "delete_tontine", "join_tontine", "invite", "accept_invitation", "reject_invitation", "add_cotisation", "send_message", "create_tour", "pay_tour"],
    required: true,
  },
...
io.on("connection", (socket) => {
  ...
  // Rejoindre une salle pour une tontine spécifique
...
async function logAction(tontineId, userId, type, description) {
  try {
    ...
    await action.save();
    await action.populate("user", "name");
    await action.populate("tontine", "name");

    // Émettre l'action via Socket.IO
    io.to(tontineId).emit("action", {
      _id: action._id,
      tontine: { _id: action.tontine._id, name: action.tontine.name },
      user: { _id: action.user._id, name: action.user.name },
      type: action.type,
...
app.post("/api/tontines", authMiddleware, async (req, res) => {
  try {
    ...
    await logAction(
      ...
      req.userId,
      "create_tontine",
      `Tontine "${tontine.name}" créée par ${req.user.name}`
    );
...
app.put("/api/tontines/:id", authMiddleware, async (req, res) => {
  try {
    ...
    await logAction(
      ...
      req.userId,
      "update_tontine",
      `Tontine "${tontine.name}" mise à jour par ${req.user.name}`
    );
...
app.delete("/api/tontines/:id", authMiddleware, async (req, res) => {
  try {
    ...
    await logAction(
      ...
      req.userId,
      "delete_tontine",
      `Tontine "${tontine.name}" supprimée par ${req.user.name}`
    );
  ...
  try {
    ...
    if (!tontine.members.includes(req.userId)) {
      ...
      await logAction(
        ...
        req.userId,
        "join_tontine",
        `${user.name} a rejoint la tontine "${tontine.name}"`
      );
    }
    res.json(tontine);
  } catch (err) {
...
app.post("/api/cotisations", authMiddleware, async (req, res) => {
  try {
    ...
    await logAction(
      ...
      req.userId,
      "add_cotisation",
      `${user.name} a ajouté une cotisation de ${amount} FCFA à la tontine "${tontine.name}"`
    );
...
app.post("/api/tontines/:id/tours", authMiddleware, async (req, res) => {
  try {
    ...
    await logAction(
      ...
      req.userId,
      "create_tour",
      `Tour créé pour ${user.name} dans la tontine "${tontine.name}"`
    );
...
app.put("/api/tours/:id/pay", authMiddleware, async (req, res) => {
  try {
    ...
    // Enregistrer l'action
    const user = await User.findById(tour.beneficiary);
    await logAction(
      tour.tontine._id,
      req.userId,
      "pay_tour",
      `Tour payé pour ${user.name} dans la tontine "${tour.tontine.name}"`
    );
...
app.post("/api/tontines/:id/messages", authMiddleware, async (req, res) => {
  try {
    ...
    await logAction(
      ...
      req.userId,
      "send_message",
      `${user.name} a envoyé un message dans la tontine "${tontine.name}"`
    );
...
app.post("/api/tontines/:id/invite", authMiddleware, async (req, res) => {
  try {
    ...
    await logAction(
      ...
      req.userId,
      "invite",
      `${fromUser.name} a invité ${user.name} à la tontine "${tontine.name}"`
    );

    // Émettre l'invitation via Socket.IO
...
app.post("/api/invitations/:id/accept", authMiddleware, async (req, res) => {
  try {
    ...
    await logAction(
      ...
      req.userId,
      "accept_invitation",
      `${user.name} a accepté une invitation pour la tontine "${tontine.name}"`
    );
...
app.post("/api/invitations/:id/reject", authMiddleware, async (req, res) => {
  try {
    ...
    await logAction(
      ...
      req.userId,
      "reject_invitation",
      `${user.name} a rejeté une invitation pour la tontine "${tontine.name}"`
    );
...
// Nouvelle route pour l'historique des actions
app.get("/api/tontines/:id/actions", authMiddleware, async (req, res) => {
  try {
    const tontine = await Tontine.findById(req.params.id);
    if (!tontine) return res.status(404).json({ error: "Tontine introuvable" });
    if (!tontine.members.includes(req.userId) && tontine.admin.toString() !== req.userId) {
      return res.status(403).json({ error: "Non autorisé" });
    }
